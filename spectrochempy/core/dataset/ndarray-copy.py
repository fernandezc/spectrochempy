...
* `label`: The label name of the data array, often a quantity name(e.g, `frequency`)
...
accessible, but has been deprecated as it is now replaced by the `label` attribute.
"""
...
class NDArray(tr.HasTraits):
    ...
    **kwargs
    ...
    label : str, optional
        ...
        It is optional but recommended giving a label to each array.
    ...
    _id = tr.Unicode()
    _name = tr.Unicode()
    _label = tr.Unicode(allow_none=True)
    ...
    def __init__(self, data=None, **kwargs):
        ...
        self.data = data
        self.label = kwargs.pop("label", kwargs.pop("title", self.label))
        if "dims" in kwargs:
    ...
    def __repr__(self):
        ...
        if not self.is_empty and self.data is not None:
            ...
            dtype = self.dtype
            data = ""
            units = f" {self.units:~K}" if self.has_units else " unitless"
            body = f"[{dtype}]{data}"
        else:
            body = "empty"
    ...
    def label(self):
        """
        Return a user-friendly name for the array label.

        When the label is provided, it can be used for labeling the object,
                ...
                # particular case of dimensionless units: absorbance and transmittance
                    ...
                    if str(oldunits) in ["transmittance", "absolute_transmittance"]:
                        if str(units) == "absorbance":
                            ...
                            new._units = units
                            if new.label and new.label.lower() == "transmittance":
                                new.label = "absorbance"
                        ...
                        if str(units) in ["transmittance", "absolute_transmittance"]:
                            ...
                            new._units = units
                            if new.label and new.label.lower() == "absorbance":
                                new.label = "transmittance"
                    else:
                        new = self._unittransform(new, units)
                        # change the label for spectroscopic units change
                        if (
