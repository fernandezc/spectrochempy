#!/usr/bin/env python3
"""
Check that all official plugins have version constraints compatible with a
given core release version.
...
def version_satisfies(version, constraints):
    ...
    for op, constraint_ver in constraints:
        if op == ">=":
            ...
            if not (version >= constraint_ver and version < (constraint_ver[0] + 1, 0, 0)):
                return False
    return True
...
def find_plugin_pyproject_tomls():
    ...
    for plugin_dir in sorted(PLUGINS_DIR.iterdir()):
        if plugin_dir.name.startswith("spectrochempy-") or plugin_dir.name == "plugin-template":
...
def main():
    ...
    for pyproject in plugins:
        ...
        print(f"{status}: {plugin_name}: spectrochempy{constraint_str} → {args.version}")
    ...
    if failed:
        ...
        print("::error::Update the spectrochempy constraint in these plugins' pyproject.toml")
